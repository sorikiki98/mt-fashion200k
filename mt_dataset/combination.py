...
class Fashion200kCombination(Fashion200k):
                ...
                if result2 is not None:
                    ...
                    if result3 is not None:
                        ...
                        if result4 is not None:
                            ...
                            result5 = self.combination_(results.copy()[:4], result4["mod_type"].copy(),
    ...
    def combination_(self, results, mod_type, add_type, n_turn):
        num_of_mods = len(mod_type)
        mod_attrs = ["color", "item", "pattern", "structure", "silhouette", "detail", "material", "style",
                     "functionality"]
        if num_of_mods > 1:
            for i in range(num_of_mods):
                mod_type1 = mod_type[i]
                ...
                for j in range(i + 1, num_of_mods):
                    ...
                    src_idx = results[-1]["target_img_id"]

                    available_mod_attrs = [attr for attr in mod_attrs if attr != mod_attr1 and attr != mod_attr2]
                    for c in recent_caption.split():
                        if c == org_word1 or c == org_word2:
                            continue
                        if c in colors and "color" in available_mod_attrs:
                            ...
                            if parent_caption in self.parent2different_colors and len(
                                    self.parent2different_colors[parent_caption]) >= 2 \
                                ...
                                mod_type.append(f"color-{n_turn}")
                                return self.add_single_turn_combination(src_idx, source_tuple, target_tuple, mod_type,
                                                                        add_type, mod_turn1, mod_turn2, mod_attr1,
                                                                        mod_attr2, n_turn)
                        if c in items and "item" in available_mod_attrs:
                            parent_caption = recent_caption.replace(c, "")
                            parent_caption = parent_caption.replace("  ", " ").strip()
                            if parent_caption in self.parent2different_items and len(
                                    self.parent2different_items[parent_caption]) >= 2 \
                                ...
                                return self.add_single_turn_combination(src_idx, source_tuple, target_tuple, mod_type,
                                                                        add_type, mod_turn1, mod_turn2, mod_attr1,
                                                                        mod_attr2, n_turn)
                        if c in pattern and "pattern" in available_mod_attrs:
                            parent_caption = recent_caption.replace(c, "")
                            parent_caption = parent_caption.replace("  ", " ").strip()
                            if parent_caption in self.parent2different_pattern and len(
                                    self.parent2different_pattern[parent_caption]) >= 2 \
                                ...
                                return self.add_single_turn_combination(src_idx, source_tuple, target_tuple, mod_type,
                                                                        add_type, mod_turn1, mod_turn2, mod_attr1,
                                                                        mod_attr2, n_turn)
                        if c in structures and "structure" in available_mod_attrs:
                            parent_caption = recent_caption.replace(c, "")
                            parent_caption = parent_caption.replace("  ", " ").strip()
                            if parent_caption in self.parent2different_structures and len(
                                    self.parent2different_structures[parent_caption]) >= 2 \
    ...
    def add_single_turn_combination(self, idx, source_tuple, target_tuple, mod_type, add_type,
                                    c_turn1, c_turn2, attr_type1, attr_type2, n_turn):
